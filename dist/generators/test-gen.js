...
exports.generateTestScaffolding = generateTestScaffolding;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
function log(level, cfg, ...args) { if (cfg.logLevel === 'silent')
    return; if (cfg.logLevel === 'info' && level === 'debug')
    return; console.log('[test-gen]', ...args); }
const toKebabCase = (s) => s.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
function ensureDir(d) { if (!fs.existsSync(d))
    fs.mkdirSync(d, { recursive: true }); }
function buildFeature(module, endpointKey, ep, fileBase) {
...
function buildStep(module, endpointKey, ep, moduleEndpoints, fileBase) {
    const isPositive = fileBase?.endsWith('-positive');
    ...
    const externalSeedModules = new Set();
    paramTokens.forEach(t => { if (t !== 'id' && seedParamModuleMap[t])
        externalSeedModules.add(seedParamModuleMap[t]); });
    ...
    externalSeedModules.forEach(m => {
        if (m === module)
            return; // avoid duplicate self import
...
async function generateTestScaffolding(userCfg = {}) {
    const cfg = {
    ...
    if (!fs.existsSync(apiSpecDir))
        throw new Error(`apiSpec directory not found: ${apiSpecDir}`);
    const apiSpec = {};
        ...
        try {
            ...
            while ((m = endpointRegex.exec(body))) {
                ...
                if (methodMatch && pathMatch) {
                    endpoints[key] = { endpoint: key, method: methodMatch[1], path: pathMatch[1] };
                }
            }
        ...
        try {
            if (fs.existsSync(filePathJs)) {
                ...
                for (const [k, v] of Object.entries(mod)) {
                    ...
                    const endpointObjs = Object.values(v).filter(x => x && typeof x === 'object' && 'method' in x && 'path' in x);
                    if (endpointObjs.length) {
                        picked = v;
                        break;
                    }
                }
            ...
            try {
                if (!tsNodeRegistered) {
                    try {
                        ...
                        log('debug', cfg, 'Failed to register ts-node', err);
                    }
                }
                ...
                log('debug', cfg, `Require TS failed for ${filePathTs}`, err);
            }
        }
        // Fallback parse
        if (!picked) {
            ...
            if (parsed) {
                picked = parsed;
                log('debug', cfg, `Parsed via fallback: ${filePathTs}`);
            }
        }
        if (picked)
            apiSpec[base] = picked;
        else
            log('debug', cfg, 'Skipping module (unable to load or parse):', base);
    });
    ...
    const utilFiles = [
        ...
        { name: 'request-helper.ts', content: (() => {
                if (!cfg.enableMock) {
    ...
    utilFiles.forEach(f => {
        ...
        if (!fs.existsSync(dest) || cfg.overwrite || (f.name === 'request-helper.ts' && cfg.forceMockUpgrade)) {
            if (cfg.dryRun)
                log('info', cfg, `(dry-run) Would write ${dest}`);
            else {
                fs.writeFileSync(dest, f.content, 'utf-8');
    ...
    Object.entries(apiSpec).forEach(([module, endpoints]) => {
        ...
        Object.entries(endpoints).forEach(([endpointKey, ep]) => {
            ...
            const stepContentPos = buildStep(module, endpointKey, ep, endpoints, `${base}-positive`);
            const existsFeaturePos = fs.existsSync(featureFilePos);
            const existsStepPos = fs.existsSync(stepFilePos);
            ...
            if (ep.method === 'POST' || ep.method === 'PUT') {
                ...
                const stepContentNeg = buildStep(module, endpointKey, ep, endpoints, `${base}-negative`);
                const existsFeatureNeg = fs.existsSync(featureFileNeg);
                const existsStepNeg = fs.existsSync(stepFileNeg);
