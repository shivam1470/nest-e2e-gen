...
function buildFeature(module, endpointKey, ep) {
...
function buildStep(module, endpointKey, ep, moduleEndpoints) {
...
async function generateTestScaffolding(userCfg = {}) {
    ...
    utilFiles.forEach(f => { const dest = path.join(utilityBase, f.name); if (!fs.existsSync(dest) || cfg.overwrite) {
        fs.writeFileSync(dest, f.content, 'utf-8');
    ...
    Object.entries(apiSpec).forEach(([module, endpoints]) => {
        ...
        Object.entries(endpoints).forEach(([endpointKey, ep]) => {
            ...
            const stepContent = buildStep(module, endpointKey, ep, endpoints);
            const existsFeature = fs.existsSync(featureFile);
            const existsStep = fs.existsSync(stepFile);
            if (!cfg.overwrite && existsFeature && existsStep) {
                log('debug', cfg, `Skip (both exist): ${featureFile}`);
                return;
            }
            if (cfg.dryRun) {
                log('info', cfg, `(dry-run) Would write: ${featureFile}`);
                log('info', cfg, `(dry-run) Would write: ${stepFile}`);
            }
            else {
                fs.writeFileSync(featureFile, featureContent, 'utf-8');
                fs.writeFileSync(stepFile, stepContent, 'utf-8');
                log('info', cfg, `Wrote: ${featureFile}`);
            }
        });
    });
